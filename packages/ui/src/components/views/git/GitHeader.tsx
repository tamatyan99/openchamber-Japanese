  ...
  return (
    <DropdownMenu>
      <Tooltip delayDuration={tooltipDelayMs}>
            ...
            >
              ...
              {!iconOnly && (
                <span className="git-identity-label min-w-0 flex-1 truncate text-left">
                  {activeProfile?.name || t('No identity')}
                </span>
              )}
        ...
        <TooltipContent sideOffset={8}>{t('Git identity')}</TooltipContent>
      </Tooltip>
      <DropdownMenuContent align="end" className="w-64">
        {identities.length === 0 ? (
          <div className="px-2 py-1.5">
            <p className="typography-meta text-muted-foreground">
              {t('No profiles available to apply.')}
            </p>
          </div>
        ) : (
    ...
    <div className="flex items-center gap-1 shrink-0">
      {onOpenHistory ? (
        <Tooltip delayDuration={useTwoRowHeader ? 300 : 1000}>
          ...
          <TooltipContent sideOffset={8}>{t('History')}</TooltipContent>
        </Tooltip>
      ) : null}
    </div>
  );
