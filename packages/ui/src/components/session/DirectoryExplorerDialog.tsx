...
export const DirectoryExplorerDialog: React.FC<DirectoryExplorerDialogProps> = ({
    ...
    try {
      ...
      if (isDesktop) {
        ...
        if (!accessResult.success) {
          toast.error(t('Unable to access directory'), {
            description: accessResult.error || t('Desktop denied directory access.'),
          });
          return;
        }
        ...
        if (!startResult.success) {
          toast.error(t('Failed to open directory'), {
            description: startResult.error || t('Desktop could not grant file access.'),
          });
          return;
        }
      }
  ...
  const dialogHeader = (
    <DialogHeader className="flex-shrink-0 px-4 pb-2 pt-[calc(var(--oc-safe-area-top,0px)+0.5rem)] sm:px-0 sm:pb-3 sm:pt-0">
      <DialogTitle>{t('Add project directory')}</DialogTitle>
      <div className="hidden sm:flex sm:items-center sm:justify-between sm:gap-4">
        <DialogDescription className="flex-1">
          {t('Choose a folder to add as a project.')}
        </DialogDescription>
        {showHiddenToggle}
      </div>
    </DialogHeader>
  ...
  const pathInputSection = (
    <div className="relative">
      <Input
        ...
        placeholder={t('Enter path or select from tree...')}
  ...
  const renderActionButtons = () => (
    <>
      ...
      >
        {isConfirming ? t('Adding...') : t('Add Project')}
      </Button>
    </>
  );
