...
export function BranchPickerDialog({ open, onOpenChange, project }: BranchPickerDialogProps) {
  ...
  const commitRename = React.useCallback(async (oldName: string) => {
    ...
    try {
      ...
      await refresh();
      cancelRename();
      toast.success(t('Branch renamed'), { description: `${oldName} -> ${newName}` });
    } catch (err) {
      toast.error(t('Failed to rename branch'), {
        description: err instanceof Error ? err.message : t('Rename failed'),
      });
      setRenamingBranchKey(null);
    }
  ...
  const handleDeleteBranch = React.useCallback(async (branchName: string) => {
    ...
    try {
      ...
      await refresh();
      toast.success(t('Branch deleted'), { description: branchName });
      setConfirmingDelete(null);
      setForceDeleteBranch(null);
    } catch (err) {
      ...
      if (/not fully merged/i.test(message) && forceDeleteBranch !== branchName) {
        ...
        toast.error(t('Branch not merged'), {
          description: t('Confirm again to force delete'),
        });
      } else {
        toast.error(t('Failed to delete branch'), { description: message });
      }
    } finally {
  ...
  const handleCreateWorktreeForBranch = React.useCallback(async (branchName: string) => {
    ...
    try {
      ...
      await refresh();
      toast.success(t('Worktree created'), { description: branchName });
    } catch (err) {
      toast.error(t('Failed to create worktree'), {
        description: err instanceof Error ? err.message : t('Create worktree failed'),
      });
    } finally {
      setCreatingWorktreeBranch(null);
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[70vh] flex flex-col overflow-hidden gap-3">
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription>
            {project ? t('Local branches for {{name}}', { name: displayProjectName(project) }) : t('Select a project')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="space-y-1">
            {!project ? (
              <div className="text-center py-8 text-muted-foreground">{t('No project selected')}</div>
            ) : loading ? (
              <div className="px-2 py-2 text-muted-foreground text-sm">{t('Loading branches...')}</div>
            ) : error ? (
              <div className="px-2 py-2 text-destructive text-sm">{error}</div>
            ) : localBranches.length === 0 ? (
              <div className="px-2 py-2 text-muted-foreground text-sm">
                {searchQuery ? t('No matching branches') : t('No branches found')}
              </div>
            ) : (
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <Tooltip delayDuration={700}>
                          ...
                          <TooltipContent side="left">
                            {hasAttachedWorktree ? t('Worktree already exists') : t('Create worktree')}
                          </TooltipContent>
                        </Tooltip>
                        ...
                        <Tooltip delayDuration={700}>
                          ...
                          <TooltipContent side="left">
                            {isProjectRootBranch ? t('Rename disabled for root branch') : t('Rename')}
                          </TooltipContent>
                        </Tooltip>
                        ...
                        <Tooltip delayDuration={700}>
                          <TooltipTrigger asChild>
                            <button
                              ...
                              aria-label={hasAttachedWorktree ? t('Delete worktree') : t('Delete')}
                            >
                              {isDeleting ? (
                                <RiLoader4Line className="h-4 w-4 animate-spin" />
                              ) : (
                          ...
                          <TooltipContent side="left">
                            {hasAttachedWorktree
                              ? isProjectRootBranch
                                ? t('Delete worktree (root branch protected)')
                                : t('Delete worktree')
                              : isCurrent
                                ? t('Delete (current branch)')
                                : isProjectRootBranch
                                  ? t('Delete disabled for root branch')
                                  : t('Delete')}
                          </TooltipContent>
                        </Tooltip>
                      </div>
                    ...
                    {!isEditing && isConfirming && !hasAttachedWorktree ? (
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <span className={cn(
                          ...
                          {isForceDelete ? t('Force delete?') : t('Delete?')}
                        </span>
                        <button
                          type="button"
