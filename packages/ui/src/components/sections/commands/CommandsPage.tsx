...
export const CommandsPage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (!commandName) {
      toast.error(t('Command name is required'));
      return;
    }

    if (!template.trim()) {
      toast.error(t('Command template is required'));
      return;
    }

    if (isNewCommand && commands.some((cmd) => cmd.name === commandName)) {
      toast.error(t('A command with this name already exists'));
      return;
    }
    ...
    try {
      ...
      if (success) {
        toast.success(isNewCommand ? t('Command created successfully') : t('Command updated successfully'));
      } else {
        toast.error(isNewCommand ? t('Failed to create command') : t('Failed to update command'));
      }
    } catch (error) {
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center text-muted-foreground">
          ...
          <p className="typography-body">{t('Select a command from the sidebar')}</p>
      ...
      <div className="mx-auto w-full max-w-3xl p-3 sm:p-6 sm:pt-8">
        ...
        <div className="mb-4 flex items-center justify-between gap-4">
          <div className="min-w-0">
            <h2 className="typography-ui-header font-semibold text-foreground truncate">
              {isNewCommand ? t('New Command') : `/${selectedCommandName}`}
            </h2>
            <p className="typography-meta text-muted-foreground truncate">
              {isNewCommand ? t('Configure a new slash command') : t('Edit command settings')}
            </p>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <section className="px-2 pb-2 pt-0 space-y-0">
            ...
            {isNewCommand && (
              <div className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-8">
                <div className="flex min-w-0 flex-col sm:w-56 shrink-0">
                  <span className="typography-ui-label text-foreground">{t('Command Name')}</span>
                </div>
            ...
            <div className="py-1.5">
              <span className="typography-ui-label text-foreground">{t('Description')}</span>
        ...
        <div className="mb-8">
          ...
          <section className="px-2 pb-2 pt-0 space-y-0">
            ...
            <div className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-8">
              <div className="flex min-w-0 flex-col sm:w-56 shrink-0">
                <span className="typography-ui-label text-foreground">{t('Override Agent')}</span>
              </div>
              <div className="flex min-w-0 flex-1 items-center gap-2 sm:w-fit sm:flex-initial">
                <AgentSelector
                  agentName={agent}
            ...
            <div className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-8">
              <div className="flex min-w-0 flex-col sm:w-56 shrink-0">
                <span className="typography-ui-label text-foreground">{t('Override Model')}</span>
              </div>
              <div className="flex min-w-0 flex-1 items-center gap-2 sm:w-fit sm:flex-initial">
                <ModelSelector
